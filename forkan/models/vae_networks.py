...
def build_network(x, x_shape, latent_dim=10, network_type='atari'):
    ...
    if network_type == 'atari':
        ...
        with tf.name_scope('encoder'):
            ...
            print('\n===== encoder')
            for n, (filters, kernel_size, stride) in enumerate(zip([32, 64], # num filter
                                                                   [2, 2], # kernel size
                                                                   [(2, 2), (2, 2)])): # strides
            ...
            fc = tf.contrib.layers.fully_connected(flat_encoder, 512, activation_fn=tf.nn.relu)
            ...
            z = mus + tf.exp(0.5 * logvars) * tf.random_uniform(tf.shape(mus))

            print('==> z {}'.format(z.shape))
            print('\n===== encoder')

        with tf.name_scope('decoder'):
            fcd = tf.contrib.layers.fully_connected(z, 512, activation_fn=tf.nn.relu)
            ...
            for n, (filters, kernel_size, stride) in enumerate(zip([64, 32],  # num filter
                                                                   [2, 2],  # kernel size
                                                                   [(2, 2), (2, 2)])):  # strides
    ...
    if network_type == 'dsprites':
        ...
        with tf.name_scope('encoder'):
            ...
            print('input: {}'.format(x.shape))
            print('\n===== encoder')
            for n, (filters, kernel_size, stride) in enumerate(zip([32, 32, 64, 64], # num filter
                                                                   [4]*4, # kernel size
                                                                   [(2, 2)]*4)): # strides
                x = tf.contrib.layers.conv2d(inputs=x,
                                                 num_outputs=filters,
            ...
            fc = tf.contrib.layers.fully_connected(flat_encoder, 256, activation_fn=tf.nn.relu)
            print('fc [ReLu] => {}'.format(fc.shape))
            print('\n===== latent-{}'.format(latent_dim
                                           ))
            mus = tf.contrib.layers.fully_connected(fc, latent_dim, activation_fn=None)
        ...
        with tf.name_scope('decoder'):
            fcd = tf.contrib.layers.fully_connected(z, 256, activation_fn=tf.nn.relu)
            print('fc [ReLu] => {}'.format(fcd.shape))
            ...
            for n, (filters, kernel_size, stride) in enumerate(zip([64, 64, 32, 32],  # num filter
                                                                   [4]*4,  # kernel size
                                                                   [(2, 2)]*4)):  # strides
                x = tf.contrib.layers.conv2d_transpose(inputs=x,
                                                       num_outputs=filters,
    ...
    assert input_shape[1:] == out.shape[1:], 'input\'s {} and ouput\'s {} shape need to match!'.format(input_shape[1:],
                                                                                                       out.shape[1:])
